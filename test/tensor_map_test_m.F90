...
module tensor_map_test_m
  ...
  private
  public :: tensor_map_test_t

  type, extends(test_t) :: tensor_map_test_t
  contains
...
contains
  ...
  pure function subject() result(specimen)
    ...
    specimen = "A tensor_map_t object"
  end function
  ...
  function write_then_read_tensor_map() result(test_passes)
    logical test_passes
    ...
    associate(tensor_map => tensor_map_t(layer="inputs", minima=[-1., 0., 1.], maxima=[1., 2., 4.]))
      ...
      associate(from_json => tensor_map_t(tensor_map%to_json()))
        test_passes = tensor_map == from_json
      end associate
#ifndef _CRAYFTN
    end associate
#endif
